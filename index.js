...
function generateAndPrintBuyOrderHTML(buyOrder, index) {
  ...
  htmlContent += `<td>${buyOrder.TransactionID}</td>`;
  htmlContent += `<td>${buyOrder.OrderDate}</td>`;
  htmlContent += `<td>${buyOrder.PaymentConfirmation.PaymentStatus}, Date: ${buyOrder.PaymentConfirmation.PaymentDate}</td>`;
  htmlContent += `<td>${buyOrder.ShippingTracking.TrackingNumber}, Estimated Delivery Date: ${buyOrder.ShippingTracking.EstimatedDeliveryDate}, Carrier Company: ${buyOrder.ShippingTracking.CarrierCompany}</td>`;
